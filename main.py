...
# print(vocab_size)
# #
# # ==================LOCAL DATASET END HERE =========================
# # character level tokenizer
#
# # =============LOADING DATASET ONLINE FROM HUGGING FACE==============
#
...
# vocab_size = len(chars)
# print(f"Cleaned vocab size: {vocab_size}")

# ============= DATASET HUGGING FACE ENDS HERE===================
...
data = torch.tensor(encode(cleaned_text), dtype=torch.long)  # converts the text that's read to tensors by encoding it
...
def get_random_chunk(dataset, split, block_size=1024, batch_size=1):
    # Select either train or validation split from the Hugging Face dataset
    data_split = dataset[split]

    # Concatenate all text into one large string (or adjust if using another dataset format)
    all_text = " ".join([item['text'] for item in data_split])

    # Determine file size equivalent (total length of the text)
    file_size = len(all_text)

    # Get a random starting position to read a chunk
    start_pos = random.randint(0, file_size - block_size * batch_size)

    # Extract a block of text starting at the random position
    block = all_text[start_pos:start_pos + block_size * batch_size]

    # Convert block to tensor (you'll need your encoding function here)
...
# def get_batch(split):
#     data = get_random_chunk(split, dataset, block_size, batch_size)
